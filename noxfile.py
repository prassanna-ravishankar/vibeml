...
shutil.rmtree('src/vibeml.egg-info', ignore_errors=True);
for f in glob.glob('src/vibeml/*.so'): os.remove(f);
for f in glob.glob('src/vibeml/*.c'): os.remove(f);
"""
...
def tests_compiled(session: nox.Session) -> None:
    ...
    session.install("-e", ".", env={"VIBEML_COMPILE_MYPYC": "1"})
